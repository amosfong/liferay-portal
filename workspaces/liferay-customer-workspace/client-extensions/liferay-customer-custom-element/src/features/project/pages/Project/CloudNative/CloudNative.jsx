...
const CloudNative = () => {
	...
	// An entry without an environmentId has not been activated yet: it only
	// carries the activation code the customer needs to activate it.
	...
	const activationCodes = [...cloudNativeEnvironments]
		.filter(
			({activationCode, environmentId}) => activationCode && !environmentId
		)
		.sort(sortByEnvironmentType);
